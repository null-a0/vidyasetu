...
const SupportConsole = () => {
    ...
    const [activeTab, setActiveTab] = useState<
        "jobs" | "audit" | "rate_limits" | "cache" | "config"
    >("jobs");
    ...
    const rateLimitQuery = useQuery({
        ...
        queryFn: () =>
            fetchSupportRateLimitEvents({ limit: 50, since_hours: 72 }),
    ...
    const rerunMutation = useMutation({
        ...
        onSuccess: async (data) => {
            ...
            await queryClient.invalidateQueries({
                queryKey: ["supportJobDetail", data.generation_id],
            });
    ...
    return (
        <DashboardLayout
            ...
            subtitle="Audit logs, job status, traces, reruns, and operational stats">
            ...
            {activeTab === "jobs" && (
                <div className="grid gap-6 lg:grid-cols-3">
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <VSelect
                                    ...
                                    onChange={(e) =>
                                        setFeatureTypeFilter(e.target.value)
                                    }
                            ...
                            <div>
                                ...
                                <VInput
                                    ...
                                    onChange={(e) =>
                                        setInstitutionIdFilter(e.target.value)
                                    }
                            ...
                            <div>
                                ...
                                <VInput
                                    value={requesterUserIdFilter}
                                    onChange={(e) =>
                                        setRequesterUserIdFilter(e.target.value)
                                    }
                                    placeholder="optional"
                                />
                            </div>
                            ...
                            {jobRows.map((j) => (
                                <button
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                        ...
                                        <VBadge
                                            variant={getJobVariant(j.status)}>
                                            {j.status}
                                        </VBadge>
                                    </div>
                                    <p className="text-xs text-muted-foreground mt-1">
                                        Updated: {formatDateTime(j.updated_at)}
                                    </p>
                                    {j.progress?.message && (
                                        <p className="text-xs text-muted-foreground mt-1 line-clamp-1">
                                            {String(j.progress.message)}
                                        </p>
                                    )}
                                </button>
                            ))}
                    ...
                    <VCard className="p-5 lg:col-span-1">
                        ...
                        {selectedJobId && jobDetailQuery.isLoading && (
                            <p className="text-sm text-muted-foreground">
                                Loading job...
                            </p>
                        )}
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="text-xs text-muted-foreground space-y-1">
                                    ...
                                    <p>
                                        Requester:{" "}
                                        <span className="text-foreground">
                                            {selectedJob.requester_user_id}
                                        </span>
                                    </p>
                                    <p>
                                        Institution:{" "}
                                        <span className="text-foreground">
                                            {selectedJob.institution_id || "-"}
                                        </span>
                                    </p>
                                    <p>
                                        Updated:{" "}
                                ...
                                {selectedJob.status === "failed" && (
                                    <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-3">
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">
                                            {typeof (
                                                selectedJob.error_details as any
                                            )?.message === "string"
                                                ? String(
                                                      (
                                                          selectedJob.error_details as any
            ...
            {activeTab === "rate_limits" && (
                <VCard className="p-5">
                    ...
                    {!rateLimitQuery.isLoading &&
                        (rateLimitQuery.data?.items?.length ?? 0) === 0 && (
                            <p className="text-sm text-muted-foreground">
                                No events.
                            </p>
                        )}
                    <div className="space-y-2">
                        {(rateLimitQuery.data?.items ?? []).map((e) => (
                            <div
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                    Retry after: {e.retry_after_seconds}s •
                                    Rule: {e.rule_max_requests}/
                                    {e.rule_window_seconds}s
                                </p>
                                <p className="text-xs text-muted-foreground mt-1">
